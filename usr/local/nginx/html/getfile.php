...
if(isset($_GET['e2'])) {
    ...
    foreach($user->categories as $category) {
        foreach($category->streams as $stream) {
            if($stream->running == 1) {
                file_put_contents('/tmp/userbouquet.favourites.tv',"#SERVICE 4097:0:1:0:0:0:0:0:0:0:http%3a//".$setting->webip."%3a".$setting->webport."/".$user->username."/".$user->password."/".$stream->id.".m3u8" ."\r\n", FILE_APPEND);
    ...
    foreach($user->categories as $category) {
        foreach($category->streams as $stream) {
            if($stream->running == 1) {
                ...
                file_put_contents('/tmp/tv_user.m3u', "http://" . $setting->webip . ":" . $setting->webport . "/" . $user->username . "/" . $user->password . "/" . $stream->id.".m3u" . "\r\n", FILE_APPEND);
            }
        }
    }
