...
echo 'bouquet="' . ($setting->user_agent ? $setting->user_agent : 'vti-Streaming') . '";';
echo 'directory="/etc/enigma2/vti.sh";';
echo 'url="http://' . $setting->webip . ':' . $setting->webport  . '/playlist.php?username=' .$username  . '&password=' .$password  . '&e2";';
echo 'rm /etc/enigma2/userbouquet."$bouquet"_.tv;';
echo 'wget -O /etc/enigma2/userbouquet."$bouquet"_.tv '. 'http://' . $setting->webip . ':' . $setting->webport  . ';';
...
echo 'echo \'#SERVICE 1:7:1:0:0:0:0:0:0:0:FROM BOUQUET "userbouquet.\'$bouquet\'_.tv" ORDER BY bouquet\' >> /etc/enigma2/new_bouquets.tv;';
